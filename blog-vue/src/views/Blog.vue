<template>
	<div>
		...
		<div class="m-content">
			<el-container>
				<el-header>
					<el-input @keyup.enter.native="search" v-model="searchTitle" class="m-search" placeholder="输入关键字后回车来查询指定文章..."
					...
					<div class="block">
						...
						<el-card class="box-card" v-for="blog in blogs" :key='blog'>
							<div slot="header" class="clearfix">
								...
								<!-- 文章编辑,复制链接,删除功能 -->
								<el-button @click="del(blog.id)" style="float: right;" type="danger" plain icon="el-icon-delete"
													 circle></el-button>
								<el-button @click="cplink(blog.id)" style="float: right; margin-right: 10px" type="success" plain
...
export default {
	...
	methods: {
		...
		cplink: function (id) {
			...
			this.$copyText(this.linkUrl).then(function () {
